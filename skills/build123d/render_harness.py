...
CAMERA_PRESETS = {
    "front": {"position": (200, 0, 0), "look_at": (0, 0, 0)},
    "top":   {"position": (0, 0, 200), "look_at": (0, 0, 0)},
    "iso":   {"position": (140, 140, 140), "look_at": (0, 0, 0)},
}
...
def render_png(result, camera: str, out_path: str, size: str) -> None:
    ...
    preset = CAMERA_PRESETS.get(camera)
    if preset is None:
    ...
    # Try f3d (preferred) then fall back with an error
    import subprocess
    pos = preset["position"]
    look = preset["look_at"]
    camera_str = f"{pos[0]},{pos[1]},{pos[2]},{look[0]},{look[1]},{look[2]},0,0,1"
    result_proc = subprocess.run(
        [
            ...
            "--output", out_path,
            "--resolution", f"{w},{h}",
            "--camera-position", f"{pos[0]},{pos[1]},{pos[2]}",
            "--camera-focal-point", f"{look[0]},{look[1]},{look[2]}",
            "--no-background",
            "--quiet",
        ],
        capture_output=True,
